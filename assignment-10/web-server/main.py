...
db_pool = sqlalchemy.create_engine(
    "mysql+pymysql://", creator=get_db_conn, connect_args={"connect_timeout": 60}
)
...
class Requests(Base):
    ...
    request_time = sqlalchemy.Column(sqlalchemy.TIMESTAMP)
    requested_file = sqlalchemy.Column(sqlalchemy.String(255))
    is_banned = sqlalchemy.Column(sqlalchemy.Boolean)
    client_country = sqlalchemy.Column(sqlalchemy.String(255))
    client_ip = sqlalchemy.Column(sqlalchemy.String(15))
    gender = sqlalchemy.Column(sqlalchemy.Enum("Male", "Female"))
    age = sqlalchemy.Column(
        sqlalchemy.Enum(
            "0-16", "17-25", "26-35", "36-45", "46-55", "56-65", "66-75", "76+"
        )
    )
    income = sqlalchemy.Column(
        sqlalchemy.Enum(
            "0-10k",
            "10k-20k",
            "20k-40k",
...
class FailedRequests(Base):
    ...
    failed_request_id = sqlalchemy.Column(
        sqlalchemy.Integer, primary_key=True, autoincrement=True
    )
    request_time = sqlalchemy.Column(sqlalchemy.TIMESTAMP, nullable=False)
    requested_file = sqlalchemy.Column(sqlalchemy.String(255), nullable=False)
    error_code = sqlalchemy.Column(sqlalchemy.Integer, nullable=False)


Base.metadata.create_all(db_pool, Base.metadata.tables.values(), checkfirst=True)
